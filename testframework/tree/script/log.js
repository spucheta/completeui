...
ygLogger.prototype.debug = function() {
  if (ygLogger.DEBUG_ENABLED) {
    ...
    for (var i = 0; i < arguments.length; i++) {
      ...
      ygLogger.logStack[ygLogger.logStack.length] = timeStamp + " ms(" + totalSeconds + ") " + dateStr + " "
...
ygLogger.disable = function() {
  ...
  try {
    ygLogger.targetEl.style.visibility = "hidden";
  } catch(e) {
  }
...
ygLogger.enable = function() {
  ygLogger.DEBUG_ENABLED = true;
  try {
    ygLogger.targetEl.style.visibility = "";
  } catch(e) {
  }
};
...
ygLogger._outputMessages = function() {
  ...
  if (ygLogger.targetEl != null) {
    ...
    for (var i = 0; i < ygLogger.logStack.length; i++) {
      ...
      var oNewElement = ygLogger.targetEl.appendChild(document.createElement("p"));
